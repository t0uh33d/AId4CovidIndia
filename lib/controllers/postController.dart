  ...
  void validatePost() async {
    if (location.value == "Click Here") {
      ...
      if (plasmaAvail.value ||
        ...
        if (plasmaAvail.value) {
          if (plasmaGroups.length != 0) {
            if (formkey.currentState.validate()) {
              await DatabaseService().uploadPost(
            ...
            Get.snackbar('Select Blood Group',
                'Select type of blood group(s) available for donation',
                backgroundColor: Colors.red, colorText: Colors.white);
          }
        } else {
          if (formkey.currentState.validate()) {
            DatabaseService().uploadPost(
                name: nameController.text,
                contact: contactController.text,
        ...
        Get.snackbar('Add Availabilty', 'Please select the availability',
            backgroundColor: Colors.red, colorText: Colors.white);
      }
    }
  }
